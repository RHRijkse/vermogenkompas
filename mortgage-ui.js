(function(){
  ...
  function showStep(focus){
    for(let i=0;i<3;i++){$('mc-step-'+i).hidden=i!==step;const li=document.querySelectorAll('.mc-steps li')[i];if(i===step)li.setAttribute('aria-current','step');else li.removeAttribute('aria-current');}
    $('mc-back').hidden=step===0;
    $('mc-next').textContent=['Verder naar hypotheek →','Verder naar woning →','Bereken mijn hypotheek →'][step];
  ...
  function input(){
    ...
    return {people:[0,...($('mc-household').value==='together'?[1]:[])].map(i=>({income:value('mc-income-'+i),status:$('mc-status-'+i).value,pension:value('mc-pension-'+i)})),singleAllowance:$('mc-single').checked,rate:value('mc-rate'),years:value('mc-years'),fixedYears:value('mc-fixed'),deductible:$('mc-deductible').value==='yes',monthlyDebt:value('mc-debt'),duo:value('mc-duo'),alimony:value('mc-alimony'),propertyValue:value('mc-value'),purchasePrice:value('mc-price'),label:$('mc-label').value,ownMoney:value('mc-money'),equity:value('mc-equity'),costs:value('mc-costs')};
  }
  function render(){
    ...
    $('mc-basis').textContent=x.propertyValue?'Begrensd door uw inkomen én de opgegeven woningwaarde.':'Op basis van inkomen. De woningwaarde moet dit bedrag nog toelaten.';
    const rows=[['Hypotheekrente / rentevast',pct(x.rate)+' / '+x.fixedYears+' jaar'],['Looptijd',x.years+' jaar'],['Ingebracht eigen geld',euro(r.funds)],['Aankoop- en financieringskosten','− '+euro(x.costs)],['Indicatieve ruimte voor koopprijs',euro(r.budget)]];
    $('mc-breakdown').replaceChildren(...rows.flatMap(([k,v])=>[node('dt',k),node('dd',v)]));
    $('mc-purchase').hidden=!x.purchasePrice;
    if(x.purchasePrice){$('mc-purchase').replaceChildren(node('strong',r.gap?'Nog '+euro(Math.ceil(r.gap))+' eigen geld nodig':'Deze koopprijs past binnen de indicatie'),node('p','Benodigde hypotheek: '+euro(r.needed)+'. Bruto maandlast daarbij: '+cents(r.neededMonthly)+'.'+(!x.propertyValue?' De marktwaarde is nog niet getoetst.':'')));}
    ...
    if(!x.costs)notes.push('U heeft geen aankoop- of financieringskosten ingevuld. Deze moeten nog van uw budget af.');
  ...
  $('mc-form').addEventListener('submit',e=>{
    ...
    if(step<2){step++;showStep(true);return;}
